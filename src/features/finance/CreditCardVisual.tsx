...
export function compactCardholderName(value: string) {
  ...
  return [
    parts[0],
    ...parts.slice(1, -1).map((part) => `${part.charAt(0).toUpperCase()}`),
...
export function CreditCardVisual({ card }: { card: CreditCard }) {
  ...
  const cardholderName = card.cardholderName ?? card.name;
  const compactHolderName = compactCardholderName(cardholderName);
  return <article className={`credit-card-visual credit-card-${institutionId}`} style={{ "--card-primary": primary, "--card-secondary": secondary, "--card-foreground": foreground } as React.CSSProperties} aria-label={`Cartão ${card.name}`}>
    ...
    <div className="credit-card-middle"><div className="credit-card-chip"><img src="/card_banner/chip.png" alt="" aria-hidden="true" /></div></div>
    <div className="credit-card-number">•••• •••• •••• {card.lastFour ?? "••••"}</div>
    <footer><span><small>{card.cardholderName ? "Titular" : "Cartão"}</small><strong className="credit-card-holder-full">{cardholderName}</strong><strong className="credit-card-holder-compact">{compactHolderName}</strong></span><span className="credit-card-branding">
